...
| Discovery and screening (4–5) | Candidate register, source/rights status, preliminary gates, three-option shortlist including custom benchmark | **In progress.** Research sweeps, register and scorecard are being produced in `docs/selection/`; the custom benchmark is the live site. | `docs/selection/` |
| Comparative branded proofs (6–8) | Common-scope proofs, scorecard, effort comparison, G1 concept decision | **Proofs in progress; G1 open.** Custom proofs in `docs/proofs/`; two labelled concept mockups in `docs/selection/proofs/`. No purchase is proposed. | `docs/selection/SELECTION_REPORT.md` |
