...
Generated by `npm run register` from `content/site.config.json` (document version 1.2, schema 1.0, last reviewed 2026-09-22). Edit the configuration, not this file.
...
| `config.travel.gettingThere` | draft | Coordinator | Airport named on the previous site; travel times deliberately omitted. Getting Here link: PRD [S6]. | 2026-09-22 | Audit IMP-15: no proximity ranking is published. Each airport carries its own approval; pending entries are prefilled from the audit's official-source register and stay unpublished until verified. Owner approval of the wording is required. |
| `config.travel.gettingThere.airports[0]` | carried-forward | Coordinator | Named on the previous site; official website from the audit source register (E06, 22 September 2026). | 2026-09-22 | Confirm scheduled service for December 2026 before guest launch (audit IMP-15). |
| `config.travel.gettingThere.airports[1]` | pending | Coordinator | Listed as an air-access option on the hotel's Getting Here page according to the audit (E05, E08); not verified from this environment. | 2026-09-22 | Not published until the coordinator verifies December 2026 service and approves the entry (audit IMP-15). |
| `config.travel.gettingThere.airports[2]` | pending | Coordinator | Listed as an air-access option on the hotel's Getting Here page according to the audit (E05, E07); not verified from this environment. | 2026-09-22 | Not published until the coordinator verifies December 2026 service (general aviation and any scheduled service) and approves the entry (audit IMP-15). |
...
| `config.story` | pending | Robert / Natalie | — | 2026-09-22 | Optional module requested by the live-site audit of 22 September 2026 (IMP-12/13). Nothing is published until enabled is true, approval.state is approved, visibility is public and every image records rightsConfirmed, subjectsApproved and publicationApproved with derivatives generated by npm run images. Intake instructions: docs/OUR_STORY_INTAKE.md. Local and CI builds render a synthetic-fixture layout preview at /story-preview.html; the deployed build never includes it. |
| `config.faqs[rsvp-when]` | draft | Robert / Natalie | PRD CONTENT-04 | 2026-09-22 | Audit F02/IMP-07: the deadline is never referred to the paper invitation alone. While rsvp.cutoffAt is null the answer says the date will be published; once set, answerWithCutoff renders the date with its time zone. |
...
| review | Airports awaiting verification | PNS, JKA are prefilled but unpublished until the coordinator verifies December 2026 service and approves them (audit IMP-15). |
...
| review | config.travel.gettingThere: draft | Audit IMP-15: no proximity ranking is published. Each airport carries its own approval; pending entries are prefilled from the audit's official-source register and stay unpublished until verified. Owner approval of the wording is required. |
| review | config.travel.gettingThere.airports[0]: carried-forward | Confirm scheduled service for December 2026 before guest launch (audit IMP-15). |
| review | config.travel.gettingThere.airports[1]: pending, not published | Not published until the coordinator verifies December 2026 service and approves the entry (audit IMP-15). |
| review | config.travel.gettingThere.airports[2]: pending, not published | Not published until the coordinator verifies December 2026 service (general aviation and any scheduled service) and approves the entry (audit IMP-15). |
...
| review | config.faqs[rsvp-when]: draft | Audit F02/IMP-07: the deadline is never referred to the paper invitation alone. While rsvp.cutoffAt is null the answer says the date will be published; once set, answerWithCutoff renders the date with its time zone. |
...
| info | config.story: Our Story not published | story.enabled=false, approval pending, visibility null. Optional module (PRD CONTENT-01, audit IMP-12/13): local and CI builds render a synthetic-fixture layout preview at /story-preview.html; the deployed build omits the section, its navigation link and every story image. Intake: docs/OUR_STORY_INTAKE.md. |
| info | config.story: pending, not published | Optional module requested by the live-site audit of 22 September 2026 (IMP-12/13). Nothing is published until enabled is true, approval.state is approved, visibility is public and every image records rightsConfirmed, subjectsApproved and publicationApproved with derivatives generated by npm run images. Intake instructions: docs/OUR_STORY_INTAKE.md. Local and CI builds render a synthetic-fixture layout preview at /story-preview.html; the deployed build never includes it. |
