...
| Hosting | GitHub Pages, source *GitHub Actions*, custom domain `robertandnatalie.wedding` (apex A records and `www` CNAME verified); HTTPS certificate approved, *Enforce HTTPS* not yet ticked (enabled by hand later on 22 September 2026; the nameservers moved to Cloudflare on 23 September with the apex records left unproxied) |
...
| RSVP state when inspected | `rsvp.mode` = `coming-soon`; no form; the backend reference implementation (`backend/`, Cloudflare Workers + D1) exists with a passing test suite and is not deployed (deployed to the owners' Cloudflare account on 23 September 2026; see `backend/README.md` "Deployment status") |
...
Recommended model: **public information site + private household RSVP service.** The static site stays on GitHub Pages (no guest data, `noindex` until the owners choose otherwise). The RSVP service is the existing `backend/` implementation on Cloudflare Workers with D1, deployed on 23 September 2026 to the first-party subdomain `api.robertandnatalie.wedding` under the owners' account; the admin routes are to be placed behind Cloudflare Access (named staff accounts, MFA by policy — not yet created, and Access is not enabled on the account as of 24 September 2026) with a transactional mail provider named in the privacy notice. Credentials are random, household-scoped, digest-stored and revocable; the session is an HttpOnly, Secure, host-scoped cookie; link tokens are read from the URL fragment and exchanged only on an explicit action. `/rsvp.html` remains the entry route and needs no HTTP redirect.
