...
| "Check the custom domain" workflow (`domain-check.yml`, manual): reads the Pages settings and GitHub's DNS health check for the custom domain and writes both to the job summary; warns when the domain is proxied or HTTPS is not enforced; fails when GitHub would not serve the site. Runbook §3 and README updated. | Owner: "confirm the site's DNS" ahead of moving the domain's nameservers to Cloudflare for the RSVP service (23 September 2026); the Pages API is not reachable from the build environment | RELEASE-01, OPS-01, OPS-02 | — |
