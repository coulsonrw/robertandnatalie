...
|---|---|---|---|
| Deployed the RSVP service into the owners' Cloudflare account (backend README steps 1–9), **partly**. Done: D1 database `rsvp` created (its id is now in `backend/wrangler.toml`), migrations 0001–0002 applied, `ceremony` and `reception` seeded from `content/site.config.json`, `CREDENTIAL_PEPPER` and `SESSION_SECRET` set (never printed), Worker uploaded, 5-minute cron registered. Blocked: the custom domain `api.robertandnatalie.wedding` could not be attached because the token has no zone edit permission, so `/health` is not reachable yet. Earlier attempts the same day were blocked by the token's start date and then by read-only permissions. `content/site.config.json` is unchanged; no guest data imported. | Owner: deploy the RSVP service following backend README steps 1–9. Do not create the Access application, set admin emails, mail provider or cutoff, or import guest data. | ARCH-06 (still partly met) | None. The owners must grant zone Workers Routes Edit and DNS Edit, then redeploy. |
