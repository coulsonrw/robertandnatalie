...
function writeAccessibilityMd(a) {
  ...
  if (!a.violations.length) L.push('None.');
    ...
    if (a.violations.some((v) => v.rule === 'scrollable-region-focusable')) L.push('- `scrollable-region-focusable` on `#dialog-slot`: the invitation dialog\'s scroll container (`.dialog-slot { overflow: auto; max-height: 100svh }`) has no `tabindex`. In this Chromium build the Tab walk did reach it (recent Chromium makes overflowing scrollers keyboard-focusable on its own), but that behaviour is engine-specific and was not verified in WebKit or Gecko here, so axe rates it serious under WCAG 2.1.1/2.1.3. Fix for the author (src/styles/site.css / scripts/templates/index.mjs): either give `#dialog-slot` `tabindex="0"` with an accessible name (for example `role="region" aria-label="Your invitation"`), or size the dialog so the card never needs to scroll.');
