...
function invitationSection(view) {
  const p = view.basePath;
  const [name1, name2] = view.couple.names;
  return `<section class="invitation-section" aria-labelledby="invitation-title" id="invitation">
  <div class="invitation-card">
    ...
    <div class="invitation-body">
      ...
      <h1 class="names" id="invitation-title"><span class="name">${esc(name1)}</span><span class="conj">${esc(view.couple.conjunction)}</span><span class="name">${esc(name2)}</span></h1>
...
export function renderIndex(view) {
  const main = `<main id="main" class="page-home">
${invitationSection(view)}
${weddingDaySection(view)}
${travelSection(view)}
${questionsSection(view)}
</main>`;
  return page({ view, currentPage: 'index', title: null, main, bodyClass: 'home' });
}
