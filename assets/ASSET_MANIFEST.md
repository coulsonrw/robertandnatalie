...
| Story photographs (`assets/story/originals/`, git-ignored; derivatives in `assets/story/derivatives/`) | Owner-supplied; per-image `rightsConfirmed`, `subjectsApproved`, `publicationApproved` in `content/site.config.json` → `story.images[]` | Robert and Natalie or the named photographer | Derivatives published only when the story is enabled, approved and public; originals never published | `photographer` credit rendered in the caption when supplied | Remove derivatives if a permission is withdrawn |
